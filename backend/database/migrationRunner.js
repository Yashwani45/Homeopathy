...
/**
 * Splits a SQL script into individual queries,
 * handling comments, quotes, and escapes.
 */
const parseQueries = (sqlText) => {
  // Remove block comments
  let cleaned = sqlText.replace(/\/\*[\s\S]*?\*\//g, "");
  ...
  for (const query of queries) {
    ...
    await new Promise((resolve, reject) => {
      conn.query(query, (err, results) => {
        if (err) {
          console.error(
            `[Migration Error] Failed executing statement:\n${query}\nError: ${err.message}`
          );
...
/**
 * Creates MySQL connection configuration.
 *
 * Aiven:
 * - Port: 20900
 * - SSL: enabled
 ...
 * - SSL: disabled
 */
const createMigrationConnection = (config) => {
  const isAiven =
    config.host &&
  ...
  return mysql.createConnection({
    host: config.host,
    port: Number(
      config.port || (isAiven ? process.env.DB_PORT || 20900 : 3306)
    ),
    user: config.user,
    password: config.password,
    ...
    ssl:
      config.ssl !== undefined
        ? config.ssl
        : isAiven && process.env.DB_SSL === "true"
        ? {
            rejectUnauthorized: false,
          }
        : undefined,
...
/**
 * Runs system migrations on a specific host.
 */
const runSystemMigrationsForHost = async (config) => {
  const dbName = process.env.DB_NAME || "super_admin_db";
  ...
  const conn = createMigrationConnection(config);
  ...
  try {
    ...
    console.log(
      `[Migration Runner] Connected to ${config.host}:${config.port || 3306}`
    );
    ...
    await new Promise((resolve, reject) => {
      conn.query(
        ...
        (err) => {
          if (err) {
            console.warn(
              `[Migration Runner] Could not run CREATE DATABASE for ${dbName}: ${err.message}`
            );

            // Aiven database may already exist
            // Continue with USE DATABASE
            return resolve();
          }
    ...
    await new Promise((resolve, reject) => {
      conn.query(`USE \`${dbName}\``, (err) => {
        if (err) {
          return reject(
            new Error(
              `Failed to select database ${dbName}: ${err.message}`
            )
          );
        }
    ...
    console.log(
      `[Migration Runner] ${dbName} migrations completed successfully on host ${config.host}.`
    );
  } finally {
    conn.end();
  }
...
/**
 * Runs system migrations on online and local hosts.
 */
const runSystemMigrations = async () => {
  const dbName = process.env.DB_NAME || "super_admin_db";

  // ============================================================
  // 1. ONLINE / AIVEN DATABASE
  // ============================================================

  console.log(
    `[Migration Runner] Running system migrations for online database: ${dbName}...`
  );
  ...
  try {
    ...
    console.error(
      `[Migration Runner] Online system migration failed:`,
      err.message
    );
  }
  ...
  if (
    ...
    console.log(
      `[Migration Runner] Running system migrations for local database: ${dbName}...`
    );
    ...
    try {
      ...
      console.error(
        `[Migration Runner] Local system migration failed:`,
        err.message
      );
    }
  }
...
const runMasterMigrations = async () => {
  const dbName =
    process.env.DB_NAME || "super_admin_db";

  console.log(
    `[Migration Runner] master_db migrations merged in ${dbName}. Skipping...`
  );
...
/**
 * Runs tenant migrations on a specific host.
 */
const runTenantMigrationsForHost = async (
  tenantDbName,
  config,
  isLocal = false
) => {
  const realDbName = isLocal
    ? tenantDbName
    : db.getRealDbName(tenantDbName);
  ...
  try {
    ...
    console.log(
      `[Migration Runner] Connected to tenant database server ${config.host}:${config.port || 3306}`
    );

    // Create tenant database
    await new Promise((resolve) => {
      conn.query(
        `CREATE DATABASE IF NOT EXISTS \`${realDbName}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`,
        (err) => {
          if (err) {
            console.warn(
              `[Migration Runner] Could not run CREATE DATABASE for ${realDbName}: ${err.message}`
            );
    ...
    await new Promise((resolve, reject) => {
      conn.query(`USE \`${realDbName}\``, (err) => {
        if (err) {
          return reject(
            new Error(
              `Failed to select database ${realDbName}: ${err.message}`
            )
          );
        }
    ...
    console.log(
      `[Migration Runner] Tenant ${realDbName} migrations completed successfully on host ${config.host}.`
    );
  } finally {
    conn.end();
  }
...
/**
 * Runs tenant migrations on online and local hosts.
 */
const runTenantMigrations = async (tenantDbName) => {
  const realDbName =
    db.getRealDbName(tenantDbName);
  ...
  try {
    await runTenantMigrationsForHost(
      tenantDbName,
      {
        host: process.env.DB_HOST || "localhost",
        port: Number(process.env.DB_PORT || 3306),
    ...
    console.error(
      `[Migration Runner] Online tenant migration for ${realDbName} failed:`,
      err.message
    );
  }
  ...
  if (
    ...
    try {
      ...
      console.error(
        `[Migration Runner] Local tenant migration for ${tenantDbName} failed:`,
        err.message
      );
    }
  }
